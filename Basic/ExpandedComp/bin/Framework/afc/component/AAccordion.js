...
AAccordion.CONTEXT = 
{
    tag: '<div data-base="AAccordion" data-class="AAccordion" class="AAccordion-Style"></div>',
...
AAccordion.prototype.insertItem = function(menuText, url, data, posItem, isPrepend, isOpenLoad)
{
	var $item = this.createItem(menuText, url, data, isOpenLoad);
...
AAccordion.prototype.createItem = function(menuText, url, data, isOpenLoad)
{
	...
	if(typeof(url)=='string') 
	{
		...
		if(!isOpenLoad)
			AView.createView($contents[0], url, this);
	}
...
AAccordion.prototype.showHideManage = function(selItem)
{
	...
	if(selItem.isOpen) 
	{
		...
		//아무 작동도 되지 않는다.
		if(!this.option.isShowToggle) return;
...
{
	var backUp = this.option.isAnimation;
	this.option.isAnimation = isAnimation;
	...
	this.option.isAnimation = backUp;
};
