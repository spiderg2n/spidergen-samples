...
ATabBar.prototype.changeIcon = function(tab, icon)
{
	this._changeIconByUrl(tab, icon);
};

ATabBar.prototype._changeIconByCss = function(tab, icon)
{
	var $iconLbl = tab.iconLbl.$ele;
	$iconLbl.addClass(this.iconMap);
	$iconLbl.css(
	{
		'background-position': this.getBgPos(icon),//(-16 * icon) + 'px 0px',
...
ATabBar.prototype._changeIconByUrl = function(tab, icon)
{
	var $iconLbl = tab.iconLbl.$ele;
	$iconLbl.css(
	{
...
ATabBar.prototype.addTab = function(tabId, title, cntr, ttMsg, icon)
{
	...
	if(this.option.isIcon)
	{
		...
		this.changeIcon(tabBtnView, icon);
		/*comps[0].$ele.css(
		{
			'background-image': 'url("' + this.iconMap + '")',//'url("Theme/img/tree_item.png")',
...
{
	if(!noClose && rTab.cntr) rTab.cntr.close();
...
ATabBar.prototype.getBgPos = function(icon)
{
	var bgPos = [];
