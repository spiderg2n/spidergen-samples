...
AWindow.updateTopWindow = function(closeCntr)
{
	...
	if(!toTopWnd && closeCntr && closeCntr.option.activePropagation)
	{
...
AWindow.makeTopWindow = function(toTopWnd, isFirst)
{
	...
	var activePropagation = toTopWnd&&toTopWnd.option.activePropagation;
	
	if(activePropagation) toTopWnd.onWillActive(isFirst);
	...
	if(activePropagation) toTopWnd.onActive(isFirst);
	...
	{
		if(activePropagation && toTopWnd.isValid() ) toTopWnd.onActiveDone(isFirst);
...
{
	//if(!AContainer.prototype.open.call(this, viewUrl, parent, left, top, width, height, !this.option.inParent)) 
	//	return false;
	
	AContainer.prototype.open.call(this, viewUrl, parent, left, top, width, height, !this.option.inParent);
	
	
	//아래 코드는 onCreate 로 옮겨짐
	/*	
	this.$ele.css( { 'overflow':this.option.overflow });
	...
	*/
	...
	if(theApp.webHistoryMgr) theApp.webHistoryMgr.pushHistory({target:this.getContainerId()});
};
